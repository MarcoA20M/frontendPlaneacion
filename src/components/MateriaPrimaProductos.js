// src/components/MateriaPrimaProductos.js - Con botones de nombres a la derecha
...
export default function MateriaPrimaProductos() {
    ...
    // Extraer nombres únicos de los productos (OMAR, VINET, CARS, etc.)
    const obtenerNombresUnicos = () => {
        const nombres = productosRelacionados.map(p => {
            ...
            if (descripcion) {
                // Buscar palabras que son nombres de productos (ej: "OMAR", "VINET", "CARS")
                const palabras = descripcion.split(' ');
                for (const palabra of palabras) {
                    // Si la palabra está en mayúsculas y tiene más de 2 letras
                    if (palabra === palabra.toUpperCase() && palabra.length > 2) {
                        return palabra;
                    }
                }
                // Si no encuentra, tomar la primera palabra
        ...
        console.log("Nombres únicos encontrados:", unicos);
        return unicos;
    };
        ...
        <div className="trazabilidad-container">
            <div className="trazabilidad-header">
                <div className="header-content">
                    ...
                    <p>Visualiza qué productos consumen cada materia prima y planifica tu inventario</p>
                </div>
                <div className="header-stats">
                    <div className="stat-badge">
                        <span className="stat-number">{materiasPrimas.length}</span>
                        <span className="stat-label">Materias Primas</span>
                    </div>
                    <div className="stat-badge">
                        <span className="stat-number">{todasLasFormulas.length}</span>
                        <span className="stat-label">Fórmulas Activas</span>
                    </div>
                </div>
            </div>
                ...
                <div className="mp-list-panel">
                    <div className="filtros-superiores">
                        ...
                        <div className="tipo-filtros">
                            ...
                            <button
                                className={`tipo-filtro-btn ${filtroTipo === "BASE" ? "active" : ""}`}
                    ...
                    {selectedMP ? (
                        <>
                            <div className="mp-detalle-header">
                                {/* Fila superior: Título a la izquierda, Filtro con botones a la derecha */}
                                <div className="mp-header-row">
                                    ...
                                    {/* FILTRO CON BOTONES DE NOMBRES A LA DERECHA */}
                                                        ...
                                                        <div className="producto-detalles">
                                                            <div className="detalle-grupo">
                                                                ...
                                                                <div className="detalle-item">
                                                                    <span className="detalle-icon">📈</span>
                                                                    <div>
                                                                        <span className="detalle-label">Consumo mensual:</span>
                                                                        <strong>{consumoMensual.toLocaleString()} {selectedMP.unidad}</strong>
                                                                    </div>
                                                                </div>
                                                            </div>
